...
@ticket_router.post(
    "/buy", status_code=status.HTTP_201_CREATED, response_model=TicketModel
)
async def buy_ticket(
    ticket_data: TicketCreateModel, session: AsyncSession = Depends(get_session)
):
    created_ticket = await ticket_service.create_ticket(ticket_data, session)
